...
pub enum SearchMode {
    ...
    Image,
    Status,
    All, // matches against every field
}
...
impl SearchInput {
    ...
    pub fn activate(&mut self) {
        self.active = true;
        // drop cursor at the end so typing continues from where we left off
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect) {
        ...
        // search active hai to block cursor dikhana hai, ratatui khud nahi dikhata
        if self.active {
        ...
        let search_widget = Paragraph::new(input_text).style(style).block(
            Block::default()
                .borders(Borders::ALL)
...
impl FilterManager {
    ...
    /// Handle key events
    pub fn handle_key(&mut self, key: Key) -> bool {
        // search open ho to wahi pehle handle karo, warna keys clash karti hai
        if self.search_input.is_active() {
            if self.search_input.handle_key(key) {
                // user ne enter maara aur query bhari hai to filter lagao
    ...
    fn apply_search_filter(&mut self) {
        ...
        match self.search_input.get_search_mode() {
            ...
            SearchMode::Status => {
                // map the typed word onto a state, unknown if it doesnt match
            ...
            SearchMode::All => {
                self.current_filter = FilterMode::ByName(query); // for now "all" just means name
            }
        }
    }

    /// wrap around the quick filter list on each press
    ...
    /// build the suggestion list from whatever containers are around
    pub fn get_suggestions(&self, containers: &[Container]) -> Vec<String> {
        let mut suggestions = Vec::new();

        // sort+dedup to get distinct images
        ...
        // cap images at 5 so the list doesnt blow up
...
pub struct AdvancedSearch {
    ...
    // None means nothing highlighted yet
...
impl AdvancedSearch {
    ...
    /// Handle key events
    pub fn handle_key(&mut self, key: Key) -> bool {
        // arrow keys move through the suggestion list, wrapping at the ends
                ...
                Key::Enter => {
                    if let Some(index) = self.suggestion_index {
                        if let Some(suggestion) = self.suggestions.get(index) {
                            // clone kyunki apply_suggestion &mut self leta hai
        ...
        // only show suggestions while the search box is open
    ...
    /// suggestions look like "image:foo" or "status:running", split on the colon
    fn apply_suggestion(&mut self, suggestion: &str) {
        if let Some(colon_pos) = suggestion.find(':') {
            ...
            let value = &value[1..]; // skip the colon itself
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect) {
        // only carve out space for suggestions if we actually have some
        if self.show_suggestions && !self.suggestions.is_empty() {
            let chunks = Layout::default()
                .direction(Direction::Vertical)
                .constraints([
    ...
    /// active if the box is open OR suggestions are showing
    pub fn is_search_active(&self) -> bool {
