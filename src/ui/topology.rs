...
struct ContainerNetInfo {
    ...
    id: String,
    ip_address: String,
    /// how many networks this container is on, used to flag multi-homed ones
    network_count: usize,
}
...
/// draws the network -> containers tree
...
impl TopologyViewer {
    ...
    pub async fn load(&mut self) -> Result<()> {
        ...
        // first pass just to tally networks-per-container
        ...
        for net in raw_networks {
            ...
            if let Some(ref cont_map) = net.containers {
                for (id, endpoint) in cont_map {
                    let cname = endpoint.name.as_deref().unwrap_or("unknown").to_string();
                    let ip = endpoint.ipv4_address.as_deref().unwrap_or("").to_string();
        ...
        for network in &self.networks {
            ...
            if network.containers.is_empty() {
                ...
                for (i, container) in network.containers.iter().enumerate() {
                    // last child gets the corner glyph so the tree closes off
