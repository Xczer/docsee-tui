...
impl CheatSheet {
    ...
    fn draw_global_commands(&self, frame: &mut Frame, area: Rect) {
        let content = vec![
            ...
            Line::from(vec![
                key("Up/Down", Color::Green),
                Span::raw(" Navigate  "),
                key("Mouse", Color::Cyan),
                Span::raw(" Click rows/tabs, scroll (if enabled in config)"),
            ]),
        ];
        frame.render_widget(
    ...
    fn draw_container_commands(&self, frame: &mut Frame, area: Rect) {
        let content = vec![
            ...
            Line::from(vec![
                key("Shift+U", Color::Green),
                Span::raw(" Bulk start  "),
                key("Shift+S", Color::Yellow),
                Span::raw(" Bulk stop  "),
                key("Shift+X", Color::Red),
                Span::raw(" Bulk delete"),
            ]),
            Line::from(""),
            section_title("  Compose Operations"),
            Line::from(vec![
                key("Shift+C", Color::Green),
                Span::raw(" Compose up  "),
                key("Shift+W", Color::Yellow),
    ...
    fn draw_images_commands(&self, frame: &mut Frame, area: Rect) {
        let content = vec![
            ...
            Line::from(vec![
                ...
                key("p", Color::Yellow),
                Span::raw(" Prune unused  "),
                key("Shift+P", Color::Cyan),
                Span::raw(" Pull image"),
            ]),
            Line::from(vec![
                key("R", Color::Green),
                Span::raw(" Run container from image"),
            ]),
            Line::from(""),
        ];
        frame.render_widget(
    ...
    fn draw_system_commands(&self, frame: &mut Frame, area: Rect) {
        let content = vec![
            ...
            Line::from(vec![
                key("Left/Right", Color::Green),
                Span::raw(" Switch views (Info/Disk/Events)  "),
                key("r", Color::Blue),
                Span::raw(" Refresh  "),
                key("Up/Down", Color::Green),
                Span::raw(" Scroll"),
    ...
    fn draw_logs_commands(&self, frame: &mut Frame, area: Rect) {
        let content = vec![
            ...
            Line::from(vec![
                ...
                Span::raw(" Word wrap  "),
                key("n", Color::Green),
                Span::raw(" Line numbers"),
            ]),
            Line::from(vec![
                key("x", Color::Cyan),
                Span::raw(" Export as text  "),
                key("X", Color::Cyan),
