...
impl VolumesTab {
    ...
    pub async fn refresh(&mut self) -> Result<()> {
        match self.docker_client.list_volumes().await {
            Ok(volumes) => {
                // keep the cursor on the same volume after reload
        ...
        match key {
            ...
            Key::DeleteItem => {
                if let Some(volume) = self.get_selected_volume() {
                    ...
                    if volume.in_use {
                        self.status_message =
                            Some(format!("Warning: Volume '{}' is in use!", name));
                        return Ok(());
                    }
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let rows: Vec<Row> = self
            ...
            .enumerate()
            .map(|(idx, volume)| {
                let mut style = if volume.in_use {
                ...
                Row::new(vec![
                    ...
                    Cell::from(relative_time(&volume.created)),
                ])
                .style(style)
            })
            .collect();
    ...
    // wraps to bottom at the top edge
