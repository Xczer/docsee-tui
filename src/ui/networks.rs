...
impl NetworksTab {
    ...
    pub async fn refresh(&mut self) -> Result<()> {
        match self.docker_client.list_networks().await {
            Ok(networks) => {
                // keep cursor on the same network across reloads
        ...
        match key {
            ...
            Key::DeleteItem => {
                if let Some(network) = self.get_selected_network() {
                    ...
                    // docker's default bridge/host/none can't be removed, block it early
    ...
    pub fn draw(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let rows: Vec<Row> = self
            ...
            .enumerate()
            .map(|(idx, network)| {
                let mut style = if network.connected_containers > 0 {
                ...
                Row::new(vec![
                    ...
                    Cell::from(relative_time(&network.created)),
                ])
                .style(style)
            })
            .collect();
    ...
    fn apply_sort(&mut self) {
        ...
        self.networks.sort_by(|a, b| {
            let cmp = match col {
                ...
                4 => std::cmp::Ordering::Equal, // Type is derived, nothing to sort on
