...
use crate::{
    ...
    events::Key,
    theme::{gauge_gradient_color, Theme},
};

/// the three panes inside the system tab
...
impl SystemTab {
    ...
    pub async fn refresh(&mut self) -> Result<()> {
        // pull whatever the event stream has queued up, cap the buffer at 500
    ...
    fn draw_info(&self, frame: &mut Frame, area: Rect) {
        let info = match &self.system_info {
            Some(info) => info,
            None => {
                let p = Paragraph::new("Loading system info...").block(
                    Block::default()
                        .borders(Borders::ALL)
                        .border_type(BorderType::Rounded),
                );
        ...
        // max(1) so we never divide by zero when there are no containers
        ...
        let container_gauge = Gauge::default()
            ...
            .label(format!("{:.0}% running", running_ratio * 100.0));
    ...
    fn draw_disk_usage(&self, frame: &mut Frame, area: Rect) {
        let usage = match &self.disk_usage {
            Some(u) => u,
            None => {
                let p = Paragraph::new("Loading disk usage...").block(
                    Block::default()
                        .borders(Borders::ALL)
                        .border_type(BorderType::Rounded),
                );
        ...
        let images_gauge = Gauge::default()
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .title(format!(" Images ({}) ", usage.images_count)),
            )
            .gauge_style(
                Style::default()
                    .fg(gauge_gradient_color(images_ratio.min(1.0)))
                    .bg(Color::DarkGray),
            )
        ...
        let containers_gauge = Gauge::default()
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .title(format!(" Containers ({}) ", usage.containers_count)),
            )
            .gauge_style(
                Style::default()
                    .fg(gauge_gradient_color(containers_ratio.min(1.0)))
                    .bg(Color::DarkGray),
            )
        ...
        let volumes_gauge = Gauge::default()
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .title(format!(" Volumes ({}) ", usage.volumes_count)),
            )
            .gauge_style(
                Style::default()
                    .fg(gauge_gradient_color(volumes_ratio.min(1.0)))
                    .bg(Color::DarkGray),
            )
        ...
        let cache_gauge = Gauge::default()
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
                    .title(format!(" Build Cache ({}) ", usage.build_cache_count)),
            )
            .gauge_style(
                Style::default()
                    .fg(gauge_gradient_color(cache_ratio.min(1.0)))
                    .bg(Color::DarkGray),
            )
    ...
    fn draw_events(&self, frame: &mut Frame, area: Rect) {
        ...
        if self.events.is_empty() {
            ...
            for event in &self.events {
                ...
                {
                    Color::Red
                } else if event.action.contains("start") || event.action.contains("create") {
                    Color::Green
                } else {
                    Color::White
                };
