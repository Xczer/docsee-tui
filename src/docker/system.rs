...
impl DockerClient {
    ...
    pub async fn disk_usage(&self) -> Result<DiskUsage> {
        ...
        if let Some(images) = df.images {
            ...
            usage.images_size = images.iter().map(|i| i.size.max(0) as u64).sum();
        }
        ...
        if let Some(volumes) = df.volumes {
            ...
            usage.volumes_size = volumes
                .iter()
                .map(|v| v.usage_data.as_ref().map(|u| u.size as u64).unwrap_or(0))
                .sum();
        }

        if let Some(build_cache) = df.build_cache {
            usage.build_cache_count = build_cache.len();
            usage.build_cache_size = build_cache.iter().map(|b| b.size.unwrap_or(0) as u64).sum();
        }
            ...
            while let Some(event_result) = stream.next().await {
                match event_result {
                    Ok(event) => {
                        ...
                        let (actor_id, actor_name) = if let Some(actor) = event.actor {
                            let id = actor
                                .id
                                .as_deref()
                                .unwrap_or("")
