...
/// one network row, ready to render
...
impl DockerClient {
    ...
    pub async fn inspect_network(&self, id: &str) -> Result<DockerNetwork> {
        // needs the turbofish, bollard can't infer the filter type here
        self.inner()
    ...
    fn format_network(&self, network: DockerNetwork) -> Result<Network> {
        ...
        // subnet/gateway are buried inside the first ipam config entry
